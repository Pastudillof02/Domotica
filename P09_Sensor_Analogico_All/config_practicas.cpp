...
const uint16_t	tiempoSubida = 7200;  

TCtrlTime 	ctrlPersiana;
...
void Config_P10_CtrlPersianas(){
	...
	ctrlPersiana.tiempoFinal		= (uint16_t *)&tiempoSubida;
	ctrlPersiana.inicio				= 0;

	mbDomoboard.clear_Actuators();

	mbDomoboard.BOTON1.mbSensorEvent = Persiana;
	mbDomoboard.BOTON1.mbActuators.push(&(mbDomoboard.PERSIANA_UP));

	mbDomoboard.BOTON2.mbSensorEvent = Persiana;
	mbDomoboard.BOTON2.mbActuators.push(&(mbDomoboard.PERSIANA_DOWN));
}

void Config_P10_garaje(){

	static uint16_t	ctrl_TiempoGaraje = 1000;
	static AsyncWait asyncWait;
	static TmbRSensores rSensores;

	asyncWait.startWaiting(&ctrl_TiempoGaraje);
	...
	rSensores.mbRSensores.clear();
	rSensores.mbRSensores.push(&(mbDomoboard.BOTON1));
	...
	mbDomoboard.PHOTOTTOR.mbSensorEvent = Garaje;
	mbDomoboard.PHOTOTTOR.rSensores = &rSensores;
...
void SelectionConfiguration(uint8_t selConf){
	...
	switch(selConf){
		...
		case P10_PER:
			DEBUGLNF("P10 Seleccionada --> Control de persianas");
			Config_P10_CtrlPersianas();
			break;

		case P10_GARAJE:
			DEBUGLNF("P10 Seleccionada --> Control de garaje");
			Config_P10_garaje();
			break;
