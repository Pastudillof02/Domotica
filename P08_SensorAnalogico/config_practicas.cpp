...
void config_P9_apt_2()
{
	clear_Actuators();

	mbDomoboard.FOTOR.limCtrlPtr.limInf = &(Aregs[MB_FOTOR_L]);
	mbDomoboard.FOTOR.limCtrlPtr.limSup = &(Aregs[MB_FOTOR_H]);
	mbDomoboard.FOTOR.mbSensorEvent = InterruptorFotoresistor;
	mbDomoboard.FOTOR.mbActuators.push(&(mbDomoboard.RELE));

	mbDomoboard.FOTOT.limCtrlPtr.limInf = &(Aregs[MB_FOTOT_L]);
	mbDomoboard.FOTOT.mbSensorEvent = InterruptorFototransistor;
	mbDomoboard.FOTOT.mbActuators.push(&(mbDomoboard.TRIAC));

	mbDomoboard.BOTON1.mbSensorEvent = InterruptorBotonesFototransistor;
	mbDomoboard.BOTON1.mbActuators.push(&(mbDomoboard.TRIAC));

	mbDomoboard.BOTON2.mbSensorEvent = InterruptorBotonesFototransistor;
	mbDomoboard.BOTON2.mbActuators.push(&(mbDomoboard.TRIAC));
}
