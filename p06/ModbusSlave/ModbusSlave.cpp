 ...
 */
void ModbusSlave::set_BankCoils(uint16_t *Regs, uint16_t *Oregs, uint16_t lengthRegs, uint16_t lengthOregs){
	CRegs = Regs;
	ORegs = Oregs;
	N_CRegs = lengthRegs;
	N_ORegs = lengthOregs;
...
int ModbusSlave::update()
{
	...
	case FC_READ_REG_DI:
		return read_discrete_registers(FC_READ_REG_DI, start_addr,query[REGS_L],ORegs);
		break;
