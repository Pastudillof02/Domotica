...
enum { 
	...
	FC_READ_INPUT_REGS 	= 	0x04,	//Read contiguous block of input analog register
...
const unsigned char fsupported[] = { FC_READ_COILS, FC_READ_REG_DI, FC_READ_REG_AO, FC_WRITE_COIL, FC_READ_INPUT_REGS,
...
void ModbusSlave::set_BankInputAnalogRegister(uint16_t *Regs, uint16_t lengthRegs)
{
	IRegs = Regs;
	N_IRegs = lengthRegs;
}
...
int ModbusSlave::read_holding_registers(unsigned char function, unsigned int start_addr,unsigned char reg_count, uint16_t *regs)
{
	int packet_size = 3;
	int status;
	unsigned int i;
...
//		return preset_multiple_registers(start_addr,query[REGS_L],query,A_Regs);
	...
	case FC_READ_INPUT_REGS:
			return read_holding_registers(FC_READ_INPUT_REGS, start_addr, query[REGS_L], IRegs);
			break;
